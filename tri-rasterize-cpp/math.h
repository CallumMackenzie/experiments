...
#define NEW_LVEC(NUM_ELEMS, PREV_NELEMS, LETTER)                         \
	...
	public:
		...
		static mat<T, 4, 4> projection_3d(T fovDeg, T aspectRatio, T near, T far)
		{
		...
		static mat<T, 4, 4> lookAt_3d(nvec<T, 3> pos, nvec<T, 3> target, nvec<T, 3> up = nvec<T, 3>{0, 1, 0})
		{
		...
		static mat<T, 4, 4> scale_3d(T x, T y, T z)
		{
		...
		static inline mat<T, 4, 4> scale_3d(const nvec<T, 3> v)
		{
			return scale_3d(v[0], v[1], v[2]);
		}

		static mat<T, 4, 4> translation_3d(T x, T y, T z)
		{
			auto m = mat<T, 4, 4>::identity();
			m[0][3] = x;
			m[1][3] = y;
		...
		static inline mat<T, 4, 4> translation_3d(const nvec<T, 3> v)
		{
			return translation(v[0], v[1], v[2]);
		}

		static mat<T, 4, 4> x_rotation_3d(T x_rad)
		{
			auto m = mat<T, 4, 4>::identity();
			m[1][1] = cosf(x_rad);
			m[1][2] = sinf(x_rad);
		...
		static mat<T, 4, 4> y_rotation_3d(T y_rad)
		{
			auto m = mat<T, 4, 4>::identity();
			m[0][0] = cosf(y_rad);
			m[0][2] = sinf(y_rad);
		...
		static mat<T, 4, 4> z_rotation_3d(T z_rad)
		{
			auto m = mat<T, 4, 4>::identity();
			m[0][0] = cosf(z_rad);
			m[0][1] = sinf(z_rad);
		...
		static inline mat<T, 4, 4> rotation_3d(T x, T y, T z)
		{
			return x_rotation_3d(x) * y_rotation_3d(y) * z_rotation_3d(z);
		}

		static inline mat<T, 4, 4> rotation_3d(nvec<T, 3> v)
		{
			return rotation_3d(v[0], v[1], v[2]);
		}

		static mat<T, 2, 2> rotation_2d(T radians)
		{
			return mat<T, 2, 2>{
		...
		static mat<T, 2, 2> scale_2d(T x, T y)
		{
			return mat<T, 2, 2>{
				{x, 0},
				{0, y}};
